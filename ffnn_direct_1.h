                ...
                class ffnn_layer {
                        public:
                                ...
                                inline void evaluate(int threadn, const T *x) {
                                        for (int i = 0; i < nnodes; ++i) {
                                                values[threadn * nnodes + i] = activation_function(nodes[i]->evaluate(x));
                                        }
                                }
                                ...
                                inline void evaluate_adjoint(int threadn, const T *x, const T *y_a, int &index, T *w_a) {
                                        ...
                                        for (int i = nnodes - 1; i >= 0; --i) {
                                                T temp_a = activation_function_derivative(nodes[i]->evaluate(x)) * y_a[i];
                                                nodes[i]->evaluate_adjoint(x, temp_a, &values_a[threadn * ninputs], index, w_a);
                                        }
                                }
        ...
        template <typename T>
                class ffnn_network {
                        ...
                        public:
                                ...
                                inline void evaluate(int threadn, const T *x, T *y) {
                                        layers[0]->evaluate(threadn, x);
                                ...
                                inline void evaluate_adjoint(int threadn, const T* x, const T *y_a, T *w_a) {
                                        int index = getNWeights();

                                        if (nlayers == 1) {
                                                layers[nlayers - 1]->evaluate_adjoint(threadn, x, y_a, index, w_a);
                                                return;
                                        }
                                        ...
                                        layers[0]->evaluate_adjoint(threadn, x, &layers[1]->values_a[threadn * layers[1]->ninputs], index, w_a);
                                }
